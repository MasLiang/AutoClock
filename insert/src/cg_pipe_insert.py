...
def determain_pipe_cgen(top_module_ast, top_flg):
    # always @(posedge clk)
    # begin
    #     if (rst_n)
    #           cgen_pre        <=      1'b0;
...
def cg_pipe_insert_single_module(module_name, top_module_ast, root_path, top_flg):
    print(module_name)
    cgen = determain_pipe_cgen(top_module_ast, top_flg)
    if cgen=="":
        return 0
    ...
    for top_def in top_module_defs:
        break
    cg_pipe_inst_gen(module_name, cgen, top_flg)
...
def cg_pipe_inst_gen(inst_name, cgen, top_flg):
    cg_list = []
    ...
    cg_list.append("reg         [2:0] ap_done_d;")
    ...
    cg_list.append("    begin")
    cg_list.append("        ap_done_d   <=      3'h0;")
    cg_list.append("    end")
    cg_list.append("    else")
    cg_list.append("    begin")
    cg_list.append("        ap_done_d   <=      {ap_done_d[1:0], ap_done};")
    cg_list.append("    end")
    cg_list.append("end")
    cg_list.append("always @(posedge ap_clk)")
    cg_list.append("begin")
...
def pipe_rpt_parser(module_name, rpt_root_path, pre_latency):
...
def cg_pipe_insert(module_name, root_path, rpt_root_path, cg_max_num, cg_max_level):
    ...
    tmp_latency, tmp_power_saved, module_type= pipe_rpt_parser(module_name, rpt_root_path, 0)
    ...
    while 1:
        ...
        for _ in range(curr_module_num):
            ...
            if len(cg_module_list)==0:
                    cg_insert_result = cg_pipe_insert_single_module(curr_module[0], top_module_ast, root_path, top_flg)
            ...
            for inst in main_module_list+other_module_list:
                ...
                if not os.path.exists(rpt_root_path+"/"+nxt_module_name+"_csynth.rpt"):
                    continue
                tmp_latency, tmp_power_saved, module_type = pipe_rpt_parser(nxt_module_name, rpt_root_path, curr_module[1])
