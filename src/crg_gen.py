...
def crg_gen(file):
    ...
    for unit_idx in range(len(clk_map_mmcm[src_clk])):
        ...
        if unit[0] == "mmcm":
            ...
            lst_assign.append(f'''{'assign    mmcm'+str(unit_idx)+'__clk_in0':<40}=    {lst_clk[0]};''')
    ...
    for unit in list(clk_map_div.keys()):
        ...
        for pll_mmcm_idx in range(len(clk_map_mmcm[src_clk])):
            ...
            if clk_map_mmcm[src_clk][pll_mmcm_idx][0]=="pll":
    ...
    for unit in list(clk_map_mux.keys()):
        ...
        for mux_in_idx in range(2):
            ...
            if clk_map_mux[unit][1][mux_in_idx] == src_clk:
                lst_assign.append(f'''{'assign    mux_'+unit+'_clk_in_'+str(mux_in_idx):<40}=    {src_clk};''')
            # connect input of mux to output of div
            if clk_map_mux[unit][1][mux_in_idx] in clk_map_div:
                lst_assign.append(f'''{'assign    mux_'+unit+'_clk_in_'+str(mux_in_idx):<40}=    div_{unit}_o;''')
            # connect input of mux to output of bypass
            if clk_map_mux[unit][1][mux_in_idx] in clk_map_bypass:
            ...
            if clk_map_mux[unit][1][mux_in_idx] in clk_map_mux:
    ...
    for clk in lst_clk[1:]:
        if clk in domains_sel_if :
            lst_port.append("   input       "+clk+"_"+domains_sel_if[clk]+",")
            lst_assign.append(f'''{'assign    mux_'+clk+'_sel':<40}=    {clk}_{domains_sel_if[clk]};''')
        lst_port.append("   output      "+clk+",")
        lst_port.append("   output      rst_n_"+clk+",")
    lst_port.append("\n")
    lst_port.append("   input       "+lst_clk[0]+",")
    ...
    lst_wire = lst_mmcm_wire + ["\n"] + lst_pll_wire + ["\n"] +lst_bufgdiv_wire + ["\n"] +lst_bufgmux_wire + ["\n"] + lst_rst_sync_wire
    lst_inst = lst_mmcm_inst + ["\n"] + lst_pll_inst + ["\n"] +lst_bufgdiv_inst + ["\n"] +lst_bufgmux_inst + ["\n"] + lst_rst_sync_inst
    lst_wfile = lst_port + ["\n"] + lst_wire + ["\n"] + lst_inst + ["\n"] + lst_assign
